var FlashBufferWriter = function () {
    var _buffer;
    var _encoding = 'utf8';
    var _endian = 'B';
    var _length = 0;
    ...
    //打包并返回Buffer对象数据
    this.pack = function (mainId, childId) {
        var packLength = 4;
        var buffer = new Buffer(_length + packLength + 1 + 1);
        var position = 0;
        switch (_endian) {
            case "B":
                buffer.writeUInt32BE(buffer.length , position);
                break;
            case "L":
        ...
        position += 4;
        buffer.writeUInt8(mainId, position);
        position += 1;
        buffer.writeUInt8(childId, position);
        position += 1;
...
module.exports = FlashBufferWriter;